...
public class AdminUserController {
    ...
    public Msg getLoginedUser(HttpServletRequest request) {
        ...
        if (currentUser.isAuthenticated()) {
            //因为登陆的时候放进去的token是String 类型的name 取回出来才可以这样转化
            String username = (String) currentUser.getPrincipal();
            AdminUser userCurrent = new AdminUser();
            userCurrent.setAdName(username);
    ...
    public Msg login(@RequestParam("userName") String userName, @RequestParam("userPwd") String pwd, Model model, HttpSession session) {
        ...
        Subject currentUser = SecurityUtils.getSubject();
        if (!currentUser.isAuthenticated()) {
            UsernamePasswordToken token = new UsernamePasswordToken(userName, Md5Tool.getMd5(pwd));
            token.setRememberMe(true);
            try {
                //Subject传递token 到对应的realm里的方法里了
                currentUser.login(token);
                Msg msg = Msg.success();
                msg.msg = "登陆成功";
                return msg;
                //这里catch到的错误就是realm中抛出的错误
                ...
                Msg msg = Msg.error();
                msg.msg = "登陆失败 系统异常" + ae.getMessage();
                return msg;
            }
        }
        Msg msg = Msg.success();
